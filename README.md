...
現在の音声入力仕様は **Groq API の利用を前提** にしています。Local Whisper や Amical Cloud を主経路として使う仕様ではありません。
...
- 音声キャプチャ: レンダラーは `getUserMedia` と AudioWorklet（16 kHz モノ）を使用し、Float32 チャンクをメインプロセスへストリーミング。設定から WebRTC の echo cancellation / noise suppression / auto gain をまとめて切り替え可能。
- フローティングウィジェット: 透明・フレームレス・常に最前面のウィンドウ。通常はマウスイベントを透過。開始/停止、波形/VAD フィードバック、通知トースト、機能フラグ付きのノートウィンドウ起動、音声入力中のライブ文字起こしプレビュー。
- 音声モデル前提: 現在の運用仕様は Groq API。`設定 -> AI モデル -> 音声` で Groq API キーを設定し、Groq Whisper Large v3 Turbo または Groq Whisper Large v3 を使う。
- 文字起こしパイプライン: 録音中に音声チャンクをストリーミング、停止時に確定。PTT は低遅延、ハンズフリーは長尺向けプロファイルを使用。Groq API で条件に応じて full-audio 最終パスを実行し、低遅延は最大 5 秒、長尺は最大 10 秒待つ。
- 品質フィルタ: VAD 対応のセグメントフィルタで無音・幻覚フレーズを除去。日本語向けプロンプト文脈、検出言語の引き継ぎ、低情報音声に限定した語彙 echo フィルタ、句読点正規化、長尺入力向けの繰り返し圧縮を追加。
- 書式化: クラウド書式化は音声認識とは別に有効化でき、選択中の書式化モデル設定を使用。
- 履歴: 完了・失敗した音声入力を音声パス、要求言語、検出言語、音声モデル、書式化モデル、メタデータとともに永続化。保持設定はアプリ設定から変更可能。
- ノート: ローカルノートは Lexical/Yjs の update 永続化と定期コンパクション。機能フラグ有効時はウィジェットからノートウィンドウを起動可能。
- 設定とオンボーディング: アプリ設定はバージョン付きローカルスキーマ（マイグレーション v11 まで）。音声入力設定では言語、自動検出、句読点、マイク、音声処理、書式化を調整可能。オンボーディングはウェルカム、権限、発見、モデル選択、完了をカバー。
...
- Groq API キー経由の音声認識（PTT は低遅延、ハンズフリーは長尺向け Whisper 文字起こし）。
- 日本語/CJK 向け Whisper プロンプト文脈、検出言語の後続チャンクへの引き継ぎ、Groq full-audio 最終パス、リトライ時の全音声一括転写。
- VAD 対応の文字起こしフィルタ、語彙 echo 誤爆防止、日本語クリーンアップ、句読点保持の改善。
- マイク音声処理（echo cancellation / noise suppression / auto gain）のオン/オフ設定。
- 完了した文字起こしのバックグラウンド永続化（ペーストフローを履歴/統計書き込みでブロックしない）。
- ローカル **Amical remake** ビルド向けのアプリブランディングとパッケージング変更。

リポジトリ状態、実装メモ、使い方、検証コマンドは [docs/customization.md](docs/customization.md) を参照してください。
